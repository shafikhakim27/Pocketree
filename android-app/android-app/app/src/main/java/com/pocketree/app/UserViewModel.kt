        ...
        client.newCall(request).enqueue(object: Callback {
            override fun onResponse(call: Call, response: Response) {
                if (response.isSuccessful) {
                    ...
                    Log.d(TAG, "Login response: $jsonResponse")  // Debug log
                    val data = gson.fromJson(jsonResponse, LoginResponse::class.java)

                    // 1. Save token
                    NetworkClient.setToken(NetworkClient.context, data.token)

                    // 2. Update ALL LiveData from login response
                    username.postValue(data.username)
                    totalCoins.postValue(data.totalCoins)
                    levelName.postValue(data.levelName)
                    tasks.postValue(data.tasks)

                    // Set defaults for fields not in login response
...
//                isLoading.postValue(false)
    ...
    fun fetchDailyTasks(){
        Log.d(TAG, "fetchDailyTasks: Starting request")
        ...
        client.newCall(request).enqueue(object : Callback {
            override fun onResponse(call: Call, response: Response) {
                ...
                Log.d(TAG, "fetchDailyTasks: Response code ${response.code}")
                Log.d(TAG, "fetchDailyTasks: Response body $responseBody")

                if (response.isSuccessful) {
                    val taskListType = object : TypeToken<List<Task>>() {}.type
                    val fetchedTasks: List<Task> = gson.fromJson(responseBody, taskListType)
                    Log.d(TAG, "fetchDailyTasks: Parsed ${fetchedTasks.size} tasks")
                    tasks.postValue(fetchedTasks)
                } else {
                    Log.e(TAG, "fetchDailyTasks: Failed with ${response.code}")
                }
            }
            override fun onFailure(call: Call, e: IOException) {
                Log.e(TAG, "fetchDailyTasks: Network error", e)
                e.printStackTrace()
            }
        })
    }
